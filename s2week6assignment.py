...
class Student:
    ...
    def enroll(self, course_code, grade):
        changed_code = course_code.upper()
        self._courses[changed_code]= grade
        return f"{self.name} enrolled in {changed_code} with grade {grade}"
    ...
    def best_course(self):
        ...
        highest_grade = 0
        h_code = None
        for code,grade in self._courses.items():
